...
<ScriptService()>
Public Class AdminSeminarRegistrationForm
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        Try
            ...
            Else
                If (transaction.CardType.ToLower() = "manual" Or transaction.CardType.ToLower() = "check") Then
                    'Skip this step if Manual or check
                    transaction.AuthNetCode = transaction.CardType
                    transaction.AuthNetTransId = Guid.NewGuid().ToString()
                    transaction.RtnMesg = ""
                    transaction.PNRef = String.Format("User: {0}", Session("username"))
                    returnPayment = True 'Have to just set to true because there is no way to confirm the payment has been collected
                Else
    ...
    <WebMethod()>
    Public Shared Function FindDoctor(ByVal doctorId As String, ByVal lastName As String) As String
        Dim returnJson As Object
        If (doctorId.Length > 0 AndAlso IsNumeric(doctorId)) Then
            returnJson = FindDoctorById(doctorId)
        Else
            returnJson = FindDoctorByLastName(lastName)
        End If
        Try
    ...
    Private Shared Function FindDoctorByLastName(ByVal lastName As String) As Object
        Dim doctor As New Doctor
        Dim doctorList As List(Of Doctor) = doctor.FindDoctorByLastName(lastName)
        If (Not (IsNothing(doctorList))) Then
            If doctorList.Count = 1 Then
                'return doctor info to client
                Return doctorList.FirstOrDefault()
            ElseIf (doctorList.Count > 1) Then
                Dim multipleDoctorError = New With {
                                        .Error = True,
                                        .ErrorMsg = "Your request returned multiple results. Please use the Doctor ID lookup instead."
                                     }
                Return multipleDoctorError
            End If
        End If
        Dim anonError = New With {
                                    .Error = True,
                                    .ErrorMsg = "No doctor record was not found. Please try again."
                                 }
        Return anonError
    End Function

    Private Shared Function FindDoctorById(ByVal doctorId As String) As Object
        Dim doctor As New Doctor
        Dim getDoctorByDoctorId As Doctor = doctor.GetDoctorByDoctorId(doctorId)
        If Not (IsNothing(getDoctorByDoctorId)) Then
            'return doctor info to client
            Return getDoctorByDoctorId
        End If
        Dim anonError = New With {
                            .Error = True,
                            .ErrorMsg = "No doctor record was not found. Please try again."
